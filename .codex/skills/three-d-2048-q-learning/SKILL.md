...
description: 在 4×4×4 三维 2048 中实现、训练、调试可复现的 Q-learning Agent；用于维护状态、动作、奖励、训练指标和游戏引擎适配器。
---
...
使用本 skill 处理 src/game_2048/agent/ 中的强化学习代码，或将 Board3D 游戏引擎接入 Agent。
不使用它修改浏览器渲染、棋子动画或游戏规则本身。

## 固定接口

- 状态 s：Board3D 的 4×4×4 方块数值；Agent 内部用 log2(tile) 编码，空位为 0。
- 动作 a：left、right、up、down、forward、back；前端中 forward/back 对应 Q/E。
- 转移：Board3DEngine 适配 Board3D，提供 reset() 与 move(action)，返回新状态、是否移动、合并得分增量。
